...
function ImageRow({ images, speed = -0.25, offset = 0 }) {
    // Double the images to the row is wide enough to never show gaps
...
export default function FeaturesSection() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            // Progress: 0 when sections enters the bottom, 1 when it leaves top
            const progress = 1 - rect.bottom / (viewH + rect.height);
            const p = Math.max(0, Math.min(1, progress));

            // Each row moves at different speeds/directions based on scroll progress
            // Scale range to viewport width so it works on all devices
